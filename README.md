...
- Write path — `dhan_place_order` and `dhan_cancel_order` verified end-to-end against a sandbox account (after deactivating the account's Kill Switch via `DhanHQ::Models::KillSwitch.deactivate`): instrument resolution, full risk pipeline, audit logging, real order placement, and real cancellation, all through the MCP-gated path. `square_off_all`/`square_off_position` remain untested — the placed order never filled (sandbox doesn't appear to simulate real matching), so there was no position to square off.
