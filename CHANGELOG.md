...
- **Live-tested**: core REST client (profile, funds, holdings, positions, orders, instrument lookup, `ltp`), MCP protocol layer (handshake, tools/list, tools/call, error codes) against both fake and real credentials, all 11 skills' intent-building logic against real option chain data (real security IDs confirmed for 8 multi-leg strategies), WebSocket streaming (`DhanHQ::WS.connect` — connected, subscribed to a real NIFTY ticker, received live ticks, clean disconnect).
