...
Verified live against a real Dhan account (both a live/production-scoped account and a sandbox account) and via an independent MCP client.

- **Live-tested, including the write path for the first time**: core REST client (profile, funds, holdings, positions, orders, instrument lookup, `ltp`), MCP protocol layer (handshake, tools/list, tools/call, error codes) against fake, real, and sandbox credentials, all 11 skills' intent-building against real option chain data, WebSocket streaming, and — via the sandbox environment — the full `dhan_place_order` and `dhan_cancel_order` paths end-to-end. Initial attempts were rejected by the account's own Kill Switch (`RMS: Trade rejected as Kill Switch is Activated`); deactivated it via `DhanHQ::Models::KillSwitch.deactivate` (a real, working API — no dashboard needed), then a real order placed successfully (`status: PENDING`, `oms: CONFIRMED`) and a real cancel succeeded through the same MCP-gated path immediately after.
- **Not tested**: `square_off_all`/`square_off_position` against a real open position. The placed order never filled (sandbox appears not to simulate real fills/matching — no counterparty), so no position ever existed to square off. This may be structurally untestable in sandbox regardless of Kill Switch state; closing it fully would need a live account with an actual small fill.
- **`Concentration`/`PositionLimits`/`MaxLoss` risk checks** have only been observed against zero-position, zero-or-low-balance accounts. Their math is unit-tested; behavior against a real non-empty portfolio has not been observed.
