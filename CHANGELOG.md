...
- **Live-tested, including the write path for the first time**: core REST client (profile, funds, holdings, positions, orders, instrument lookup, `ltp`), MCP protocol layer (handshake, tools/list, tools/call, error codes) against fake, real, and sandbox credentials, all 11 skills' intent-building against real option chain data, WebSocket streaming, and — via the sandbox environment — the full `dhan_place_order` path end-to-end: instrument resolution, every `Risk::Pipeline` check, the `OrderAudit` audit log, and a real Dhan API request that reached genuine order validation/matching. The only thing stopping a real fill in that sandbox test was the account's own Kill Switch toggle (`RMS: Trade rejected as Kill Switch is Activated`) — an account-level safety control, not a gem defect.
- **Not tested**: a *filled* order (Kill Switch was active throughout sandbox testing), `dhan_cancel_order`, `square_off_all`, `square_off_position` against a real open position — there were none to act on. The code paths are unit-tested with mocks; the live gap is now narrower (order submission proven) but position-closing flows remain unverified against a real fill.
